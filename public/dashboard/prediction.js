...
function validateInput(temperature, turbidity, ph, dsolids, doxygen) {
  ...
  if (
    ...
    document.getElementById("model-output").textContent =
      "Value of WQPs can't be empty.";
    validation = false;
  }

  if (!(temperature >= -50 && temperature <= 100)) {
    tempMsg.innerText = "Temperature value should be in range -50 to 100 °C";
    tempMsg.style.display = "block";

    validation = false;
  }
  if (!(turbidity >= 0 && turbidity <= 1100)) {
    turbidityMsg.innerText = "Turbidity value should be in range 0 to 1100 NTU";
  ...
  if (!(dsolids >= 0 && dsolids <= 100000)) {
    dsolidsMsg.innerText =
      "Dissolved Solids value should be in range 0 to 100000 mg/l";
    validation = false;
    dsolidsMsg.style.display = "block";
  }
  if (!(doxygen >= 0 && doxygen <= 50)) {
    doxygenMsg.innerText =
      "Dissolved Oxygen value should be in range 0 to 50 mg/l";
    validation = false;
